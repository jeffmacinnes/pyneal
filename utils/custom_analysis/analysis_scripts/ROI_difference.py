...
custom stat script to calculate the difference between 2 ROIs

Note: ROI-1 > ROI-2, so choose your ROI's in the proper order in the GUI
...
class CustomStatClass:
	...
	def __init__(self, coordinates):
		...
		# (FEEL FREE TO COMMENT OUT IF THIS SCRIPT SPECIFIES ITS OWN ROI COORDS)
		...
		# preallocate arrays to store ROI values for each volume
	...
	def compute_stats(self, volume):
		...
		# compute difference between ROIs 
		self.stat_output = self.r1_vals[self.volume_count] - self.r2_vals[self.volume_count]
